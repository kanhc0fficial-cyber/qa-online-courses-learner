...
class AudioProcessor:
    def __init__(self, 
        ...
        try:
            ...
            self.device = device
            # int8 is substantially faster and smaller on CPU while retaining
            # the multilingual medium model needed for Chinese/English speech.
            ...
            try:
                ...
                self.language = language if language else None
                self.initial_prompt = initial_prompt
                self.model = whisper.load_model(model_size_or_path, device=device)
                self.has_ffmpeg = True
                logger.info(
                    "Loaded official multilingual Whisper model %s on %s",
                    model_size_or_path,
                    device,
                )
            except Exception as whisper_error:
                logger.error("Could not load either Whisper backend: %s", whisper_error)
                raise RuntimeError("No usable local Whisper backend is available") from whisper_error
        ...
        try:
            if self.backend == "openai-whisper":
                ...
                return AudioTranscript(
                    text=" ".join(str(segment.get("text", "")).strip() for segment in segment_data).strip(),
                    segments=segment_data,
                    language=str(result.get("language", self.language or "unknown")),
                )

            # faster-whisper path with VAD filtering
            segments, info = self.model.transcribe(
                str(audio_path), beam_size=5, word_timestamps=True, vad_filter=True,
