...
class PromptLoader:
    ...
    def _find_prompt_file(self, prompt_path: str) -> Path:
        ...
        # An explicitly configured directory must override packaged defaults.
        if self.prompt_dir:
            user_path = self.prompt_dir
            full_path = user_path / prompt_path if user_path.is_absolute() else Path.cwd() / user_path / prompt_path
            if full_path.exists():
                return full_path
