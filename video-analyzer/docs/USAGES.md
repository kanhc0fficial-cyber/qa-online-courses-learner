...
| `--keep-frames` | Keep extracted frames after analysis | True | `--keep-frames` |
| `--discard-frames` | Delete extracted frames after analysis | False | `--discard-frames` |
...
- `frames.per_minute`: Local uniform scan density per minute (no API cost)
- `frames.scene_threshold`: FFmpeg scene-change threshold
- `frames.black_threshold`: Mean brightness below which a frame is discarded
- `frames.hash_distance`: dHash Hamming distance used for near-duplicate removal
- `frames.max_count`: Hard limit on frames sent for analysis
- `recording.group_size`: Consecutive images sent in each multimodal request (default: 6)
...
- `keep_frames`: Retain extracted frames; enabled by default so the record remains auditable
